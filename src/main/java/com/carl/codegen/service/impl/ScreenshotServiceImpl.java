...
public class ScreenshotServiceImpl implements ScreenshotService {
    ...
    public String generateAndUploadScreenshot(String url) {
        ThrowUtils.throwIf(StrUtil.isBlank(url), ErrorCode.PARAMS_ERROR, "网页URL不能为空");
        log.info("开始生成网页截图，URL: {}", url);
        // 1. 生成本地截图
        String localScreenshotPath = ScreenshotUtils.saveWebPageScreenshot(url);
        ThrowUtils.throwIf(StrUtil.isBlank(localScreenshotPath), ErrorCode.OPERATION_ERROR, "本地截图生成失败");
        try {
            // 2. 上传到对象存储
            String cosUrl = uploadScreenshotToCos(localScreenshotPath);
            ThrowUtils.throwIf(StrUtil.isBlank(cosUrl), ErrorCode.OPERATION_ERROR, "截图上传对象存储失败");
            log.info("网页截图生成并上传成功: {} -> {}", url, cosUrl);
            return cosUrl;
        } finally {
            // 3. 清理本地文件
    ...
    /**
     * 生成截图的对象存储键，格式：/screenshots/yyyy/MM/dd/filename.jpg
     */
