...
import com.carl.codegen.langgraph4j.ai.CodeQualityCheckService;
...
public class CodeQualityCheckNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            try {
                // 1. 读取并拼接代码文件内容
                String codeContent = readAndConcatenateCodeFiles(codeOutputDir);
                if (StrUtil.isBlank(codeContent)) {
                    ...
                    // 2. 调用 AI 进行代码质量检查
                    CodeQualityCheckService service = SpringContextUtil.getBean(CodeQualityCheckService.class);
                    qualityResult = service.checkCodeQuality(codeContent);
